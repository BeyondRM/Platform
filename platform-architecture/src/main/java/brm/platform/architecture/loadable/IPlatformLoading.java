...
/**
 * <h2>IPlatformLoading</h2>
 * The platform-loading method declarations.
 * <p/>
 ...
 */
abstract public class IPlatformLoading {
  /**
   ...
   * @see IPlatformLoading
   */
  abstract public void beforeInitialization(File f);
   ...
   * </ul>
   * For most Platform modules, this step is meant to be completed quickly.
   * <p/>
   * We return to our initialization state to iterate through the next platform object. After all platform objects have
   * finished this step, we call {@link #initializeDuring(ProgressBar) initializeDuring(ProgressBar)} to perform any
   * remaining tasks necessary.
   ...
   * </ul>
   * The implementation of this method definition in each platform module is dependent upon the complexity of the data
   * stored within the files. In the editor, we read and save in plain-text resources, where possible; but in order to
   * not just simply deliver our games with the game assets as plain-text, we can make much of it encoded as the bytes
   * in the RGBA values of a graphics pixel. If security is a concern for some, the game maker could possibly wrap the
   * data files in additional encryption.
   * <p/>
   * We return to our initialization state to iterate through the next platform object. After all platform objects have
   * finished this step, we call {@link #initializeFinish(ProgressBar) initializeFinish(ProgressBar)} to perform any
   * remaining tasks necessary.
   * @param pb A {@link ProgressBar} object, representing an updating progress bar.
   * @see IPlatformLoading
   * @see #initializedCount initializedCount
   ...
   * <p/>
   * We return to our initialization state to iterate through the next platform object. After all platform objects have
   * finished this step, we would be considered "loaded" and "validated". The final step, if this is not a battle test,
   ...
   * <p/>
   * "Platform validation" may be dependent upon the module instance and intent of the loading; but for most reasons, a
   * validation method will attempt to ensure that the object is present and utilizable, and its object members can be
   * likewise validated.
   * @see IPlatformLoading
   */
